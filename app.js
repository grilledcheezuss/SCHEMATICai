...
class AnalysisManager {
    ...
    static detectFields(textContent, viewport) {
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            if (/CUSTOMER|CLIENT|COMPANY|OWNER/i.test(text) && !fields.customer) {
                const nextItem = items[i + 1];
                if (nextItem && nextItem.str.trim().length > 2) {
                    const nextNorm = normalize(nextItem.transform[4], nextItem.transform[5]);
                    fields.customer = {
                        label: { text: item.str, x: normalized.x, y: normalized.y },
                        value: { text: nextItem.str, x: nextNorm.x, y: nextNorm.y }
                    };
                }
            }
            
            if (/JOB\s*(NO|NUMBER|#|NAME)|PROJECT\s*(NO|#|NAME)/i.test(text) && !fields.jobNo) {
                const nextItem = items[i + 1];
                if (nextItem && nextItem.str.trim().length > 1) {
                    const nextNorm = normalize(nextItem.transform[4], nextItem.transform[5]);
                    fields.jobNo = {
                        label: { text: item.str, x: normalized.x, y: normalized.y },
                        value: { text: nextItem.str, x: nextNorm.x, y: nextNorm.y }
            ...
            if (/DATE|SUBMITTED|REVISION/i.test(text) && !fields.date) {
                const nextItem = items[i + 1];
                if (nextItem && (nextItem.str.includes('/') || nextItem.str.includes('-') || /\d{4}/.test(nextItem.str))) {
                    const nextNorm = normalize(nextItem.transform[4], nextItem.transform[5]);
                    fields.date = {
                        label: { text: item.str, x: normalized.x, y: normalized.y },
                        value: { text: nextItem.str, x: nextNorm.x, y: nextNorm.y }
            ...
            if (/CP-\d{4}/i.test(text) && !fields.panelId) {
                fields.panelId = {
    ...
    static exportResults() {
        ...
        a.href = url;
        a.download = `pdf-analysis-${this.analyzed}-pdfs-${Date.now()}.json`;
        a.click();
